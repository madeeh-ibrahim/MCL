// mcl_hd_throughput.cpp — Doc ID MCL-HD-THROUGHPUT-2026-0817-001
...
int main(){
  ...
  // warmup
  for (int i = 0; i < 5; i++) { auto d = derive_child(S,3,5,i); sink += d.p; }
  const int NB = 200;
  auto t0 = Clock::now();
  for (int i = 0; i < NB; i++) { auto d = derive_child(S,3,5,i); sink += d.p + d.q; }
  auto t1 = Clock::now();
  double us_bare = std::chrono::duration<double,std::micro>(t1-t0).count()/NB;
  const int NS = 50;
  auto t2 = Clock::now();
  for (int i = 0; i < NS; i++) { auto d = derive_child_safe(S,3,5,i); sink += d.p + d.q; }
  auto t3 = Clock::now();
