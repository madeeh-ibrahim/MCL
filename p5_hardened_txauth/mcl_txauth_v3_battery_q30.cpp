 ...
 * MCL_T4_Q30 (twelve Q30 integer weights via mcl_t4_q30_params_from_key, no floating
 * point) instead of the double-precision MCL_T4, plus T0: a same-platform determinism
 * gate (1,000 recomputations of one tag, all bit-identical).
 * ============================================================================
...
int main(int argc, char** argv) {
    ...
    std::printf("  build architecture: x86_64\n");
#elif defined(__aarch64__)
    std::printf("  build architecture: arm64\n");
#endif
    ...
    // --- Test 0: same-platform determinism gate (Q30 is bit-exact by construction) ---
    {
        Tx t=tx_of(1); Tag ref=tag_v3(A,t,V1); int same=0;
        for (int i=0;i<1000;i++) same += (tag_v3(A,t,V1)==ref) ? 1 : 0;
        std::printf("T0 determinism: %d/1000 recomputations bit-identical\n", same);
        check("T0 1000/1000 recomputations bit-identical", same==1000);
    }
    // --- Test 1: uniqueness (5,000 distinct transactions) ---
    {
        ...
        { uint8_t fp[32]; std::vector<uint8_t> cat; cat.reserve(tags.size()*32);
          for (const Tag& tg : tags) cat.insert(cat.end(), tg.begin(), tg.end());
          sha256_portable(cat.data(), cat.size(), fp);
          std::printf("T1 fingerprint: SHA-256 over the 5000 concatenated tags = ");
          for (int i=0;i<32;i++) std::printf("%02x", fp[i]); std::printf("\n"); }
    ...
    std::printf("weak-key re-draw census: %llu of %llu weight derivations had a reachable symmetry in their RAW weight set (re-drawn deterministically by the sidecar)\n", (unsigned long long)g_redraws.load(), (unsigned long long)g_derivations.load());
