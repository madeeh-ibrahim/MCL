...
#include "mcl_hd_v2.hpp"   // derive_child_v2 / derive_child_safe_v2 (one-way index mixing)
#include <cstdlib>
#include <cstring>
#include <ctime>
#include <set>
...
static const char* DOC_VERSION = "2.0.0-hdv2";
static const char* DOC_ID      = "MCL-HD-VERIFY-V2-2026-0905-001";
...
// ============================================================================
// HD DERIVATION: derive_child_v2(), derive_child_safe_v2(), DerivedKey
...
static void test_01_derivation_correctness() {
    ...
    for (int i = 0; i < NC; i++) {
        DerivedKey dk = derive_child_v2(seed, pp, qp, i);
        bool pnq = (dk.p != dk.q);
...
static void test_03_sibling_independence() {
    ...
    for (int i = 0; i < NS; i++) {
        sibs[(size_t)i] = derive_child_v2(seed, pp, qp, i);
...
static void test_04_one_way() {
    ...
    DerivedKey child = derive_child_v2(seed, pp, qp, 0);
    ...
    for (int64_t tp = T4_RANGE_MIN; tp <= range_max; tp++) {
        for (int64_t tq = T4_RANGE_MIN; tq <= range_max; tq++) {
            if (tp == tq) continue;
            DerivedKey tc = derive_child_v2(seed, tp, tq, 0);
...
static void test_05_tree_depth3() {
    ...
    for (int b = 0; b < NB; b++) {
        DerivedKey bk = derive_child_v2(seed, pm, qm, b);
        ...
        for (int d = 0; d < ND; d++) {
            DerivedKey dk = derive_child_v2(seed, bk.p, bk.q, d);
...
static void test_06_resonance_avoidance() {
    ...
    for (int i = 0; i < NDR; i++) {
        DerivedKey dk = derive_child_safe_v2(seed, pp, qp, i, max_val, Kr);
        if (dk.valid) {
...
static void test_07_reproducibility() {
    ...
    DerivedKey first = derive_child_v2(DEFAULT_SEED, DEFAULT_P, DEFAULT_Q, CHILD_IDX);
    int mm = 0;
    for (int t = 0; t < NT; t++) {
        DerivedKey dk = derive_child_v2(DEFAULT_SEED, DEFAULT_P, DEFAULT_Q, CHILD_IDX);
...
static void test_08_multi_function() {
    ...
    DerivedKey child = derive_child_v2(DEFAULT_SEED, DEFAULT_P, DEFAULT_Q, 0);
...
static void test_10_negative_control() {
    ...
    // Child != parent
    DerivedKey dk = derive_child_v2(DEFAULT_SEED, DEFAULT_P, DEFAULT_Q, 0);
