...
// P4 review measurement 2026-09-05, round 5 item R5-5 (part 2): WEAK-PAIR INSTANCES of VDF128-T4 v3 — one pair with BOTH lanes small.
...
int main(int argc, char** argv) {
    std::printf("VDF128-T4 v3 weak-PAIR instance probe (MCL-VDF128-WEAKPAIR-2026-0905-001; engine %d.%d.%d)\n", MCL_VERSION_MAJOR, MCL_VERSION_MINOR, MCL_VERSION_PATCH);
    if (argc > 2 && std::string(argv[1]) == "--grind") {
        ...
        for (uint64_t i = 40000000ull; i < 40000000ull + cap; i++) {
            ...
            MCL_Q30_Sextet W = mcl_vdf128v3_weights((const uint8_t*)buf, (size_t)L); uint32_t w[12]; lanes(W, w);
    ...
    const char* X = "weak-lane-v3-1229";   // real input whose q13 = 31590 already (< 2^16)
    MCL_Q30_Sextet W0 = mcl_vdf128v3_weights((const uint8_t*)X, std::strlen(X));
