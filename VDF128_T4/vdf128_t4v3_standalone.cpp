...
int main(int argc, char** argv) {
    ...
    // ---- Vector 5
    const char* xs = "VDF128-T4-KAT-01"; const uint8_t* x = (const uint8_t*)xs; size_t xl = std::strlen(xs); const uint64_t B = 10000, N = 1000; const int64_t kp = k_phase(12.0);
    ...
    const char* want_y = "75d76880369509b44a709e1f33c350f257e76eae29de7cc537f0a05113321273";
    bool ok = hex(y,32) == want_y && s.t[0] == 0x57e5f261u && s.t[1] == 0x0fc5236au && s.t[2] == 0x99dd6105u && s.t[3] == 0x9236016eu;
    std::printf("Vector 5 (v3): %s\n", ok ? "REPRODUCED (final state and y match the paper)" : "MISMATCH");
    return ok ? 0 : 1;
}
