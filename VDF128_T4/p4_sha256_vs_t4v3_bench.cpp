...
// P4 review measurement 2026-09-05 (round 5, R5-6): per-iteration cost of VDF128-T4 v3 against a SHA-256 hash chain on the SAME host,
...
int main(){
    ...
    const MCL_Q30_Sextet W = mcl_vdf128v3_weights((const uint8_t*)X, XL); const int64_t kp = mcl_q30_K_phase(K_DEFAULT);
    ...
    std::printf("BEST-OF-5  VDF128-T4 v3 iterate: %.1f ns (%.1f M/s)   SHA-256 chain, ARMv8 SHA-2 instructions: %.1f ns (%.1f M/s)   SHA-256 chain, system library: %.1f ns (%.1f M/s)   [t1 %08x, a0 %02x]\n",
