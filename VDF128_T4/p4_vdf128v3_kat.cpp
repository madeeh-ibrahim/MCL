...
// P4 R4 (2026-09-05): complete x -> y known-answer vector for VDF128-T4 v3 (per-input weights)
...
int main(){
  const char* xs="VDF128-T4-KAT-01"; const uint8_t* x=(const uint8_t*)xs; size_t xl=std::strlen(xs); const uint64_t B=10000, N=1000;
  std::printf("VDF128-T4 v3 KNOWN-ANSWER VECTOR (engine %d.%d.%d; per-input weights; B=%llu, N=%llu, K=%.1f)\n",MCL_VERSION_MAJOR,MCL_VERSION_MINOR,MCL_VERSION_PATCH,(unsigned long long)B,(unsigned long long)N,K_DEFAULT);
  std::printf("  x (ASCII)             \"%s\"  (%zu bytes)\n",xs,xl);
  uint8_t hx[32]; mcl_sha256(x,xl,hx); hex("h = SHA-256(x)",hx,32);
  MCL_Q30_Sextet W=mcl_vdf128v3_weights(x,xl);
  ...
  put32(0,s.t1);put32(4,s.t2);put32(8,s.t3);put32(12,s.t4); std::memcpy(buf+16,hx,32); for(int i=0;i<8;i++) buf[48+i]=(uint8_t)(N>>(i*8)); static const char otag[24]="VDF128-T4-v3-out\0\0\0"; std::memcpy(buf+56,otag,24);
  hex("SHA-256 preimage (80 B)",buf,80);
  uint8_t y[32]; mcl_sha256(buf,80,y); hex("y = SHA-256(preimage)",y,32);
  uint8_t y2[32]; mcl_vdf128v3_eval(x,xl,N,y2,B); std::printf("  API mcl_vdf128v3_eval: %s\n", std::memcmp(y,y2,32)==0?"IDENTICAL":"DIFFER");
  uint8_t y3[32]; mcl_vdf128v3_eval(x,xl,100000,y3,B); hex("y (same x, N=100000)",y3,32);
  ...
  for(int in=0;in<NI;in++){ char xi[40]; std::snprintf(xi,40,"VDF128v2-avalanche-%d",in); const uint8_t* xp=(const uint8_t*)xi; size_t xpl=std::strlen(xi);
    MCL_Q30_Sextet Wi=mcl_vdf128v3_weights(xp,xpl); VDF128_State c0=mcl_vdf128_init(xp,xpl); for(uint64_t i=0;i<B;i++) mcl_q30t4_iterate_raw(c0.t1,c0.t2,c0.t3,c0.t4,Wi,kp);
  ...
  for(int t=0;t<cnt;t++){ uint8_t a[16],b[16]; for(int i=0;i<16;i+=8){uint64_t r=sm(z); std::memcpy(a+i,&r,8);} std::memcpy(b,a,16); int bit=(int)(sm(z)%128); b[bit/8]^=(uint8_t)(1u<<(bit%8));
    VDF128_State ia=mcl_vdf128_init(a,16), ib=mcl_vdf128_init(b,16); int hi=ham(ia,ib); uint8_t ya[32],yb[32]; mcl_vdf128v3_eval(a,16,N,ya,B); mcl_vdf128v3_eval(b,16,N,yb,B); int ho=ham256(ya,yb);
  ...
  { const char* x2="VDF128-T4-KAT-00"; VDF128_State i1=mcl_vdf128_init(x,xl), i2=mcl_vdf128_init((const uint8_t*)x2,16); uint8_t ya[32],yb[32]; mcl_vdf128v3_eval(x,xl,N,ya,B); mcl_vdf128v3_eval((const uint8_t*)x2,16,N,yb,B);
