/* mcl_vdf128v2_xplat.cpp — Doc ID MCL-VDF128-XPLAT-2026-0905-002 (v2 of MCL-VDF128-XPLAT-2026-0817-001)
 * Cross-platform bit-exactness fingerprint for VDF128_T4 v2 (per-input weights).
...
int main(){
    ...
    MCL_Q30_Sextet W = mcl_vdf128v2_weights((const uint8_t*)x, std::strlen(x));
    ...
    VDF128_State sB = mcl_vdf128v2_eval_state((const uint8_t*)x, std::strlen(x), 0);
    std::printf("B  %08x %08x %08x %08x\n", sB.t1,sB.t2,sB.t3,sB.t4);
    VDF128_State sF = mcl_vdf128v2_eval_state((const uint8_t*)x, std::strlen(x), 100000);
    std::printf("F  %08x %08x %08x %08x\n", sF.t1,sF.t2,sF.t3,sF.t4);
    uint8_t y[32]; mcl_vdf128v2_eval((const uint8_t*)x, std::strlen(x), 100000, y);
    std::printf("Y  "); for(int i=0;i<32;i++) std::printf("%02x", y[i]); std::printf("\n");
    const MCL_Q30_Sextet WW = mcl_vdf128v2_weights((const uint8_t*)"xplat-fold", 10);
    ...
    uint32_t acc = 0;
    for (int k = 0; k < 8; k++) { char xi[32]; std::snprintf(xi,sizeof xi,"xplat-%d",k); uint8_t yy[32]; mcl_vdf128v2_eval((const uint8_t*)xi, std::strlen(xi), 25000, yy); for (int b = 0; b < 32; b++) acc = (acc*16777619u) ^ yy[b]; }
