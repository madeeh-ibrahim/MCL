// mcl_vdf128v2_cyclecheck.cpp — Doc ID MCL-VDF128-CYCLE-2026-0905-002 (v2 of MCL-VDF128-CYCLE-2026-0817-001)
//  Budgeted Brent cycle probe on VDF128-T4 v2 (per-input weights): 3 inputs, budget 2^33 each,
...
int main(){
  ...
  for (int c=0;c<3;c++){
    const MCL_Q30_Sextet W = mcl_vdf128v2_weights((const uint8_t*)xs[c], strlen(xs[c]));
  ...
  { const char* x = "MCL-VDF128-KAT-1";
    VDF128_State sF = mcl_vdf128v2_eval_state((const uint8_t*)x, strlen(x), 1000);
    printf("TEST5 KAT v2 final state (N=1e3): %08x %08x %08x %08x\n", sF.t1,sF.t2,sF.t3,sF.t4);
    uint8_t y[32]; mcl_vdf128v2_eval((const uint8_t*)x, strlen(x), 1000, y);
