/*
 * ============================================================================
 * mcl_vdf128v2_bench.cpp — Paper-4 Eval/Verify benchmark (CPU), VDF128-T4 v2 (per-input weights)
 * Doc ID: MCL-VDF128-BENCH-2026-0905-002 (v2 of MCL-VDF128-BENCH-2026-0821-001)
 * ============================================================================
 * WHY
...
int main() {
    const uint8_t x[] = "MCL-VDF128-T4 v2 benchmark input 2026-09-05";
    const size_t xlen = sizeof(x) - 1;
    const MCL_Q30_Sextet W = mcl_vdf128v2_weights(x, xlen);   // v2: per-input map F_x
    const int64_t kp = mcl_q30_K_phase(K_DEFAULT);
    const uint64_t B = 10000;

    std::printf("================================================================\n");
    std::printf("  VDF128_T4 v2 — Eval/Verify CPU benchmark\n");
    std::printf("  MCL-VDF128-BENCH-2026-0905-002   engine: mcl_core v%s + sidecar\n",
                MCL_VERSION_STRING);
    std::printf("  HW: %s\n", hw_string().c_str());
    std::printf("================================================================\n\n");
    ...
    uint8_t y_eval[32];
    {
        ...
        end_state = s;
        mcl_vdf128v2_output(s, x, xlen, N, y_eval);
    ...
    for (int k : ks) {
        ...
        uint8_t y_v[32];
        mcl_vdf128v2_output(cps[(size_t)k], x, xlen, N, y_v);
    ...
    for (uint64_t n : {10000ull, 1000000ull, 10000000ull, 100000000ull}) {
        uint8_t y[32];
        auto t0 = clk::now(); mcl_vdf128v2_eval(x, xlen, n, y); auto t1 = clk::now();
