...
// TEXT (VDF128-T4 v2, per-input weights, 2026-09-05) with no engine code: its own SHA-256, KDF, weight derivation, table loader, four-oscillator
...
static Weights input_weights(const uint8_t hx[32]) {   // v2: W_x = params_from_key(SHA-256(x), challenge 0)
    uint8_t info[8] = {0}; uint8_t kd[96]; kdf(hx, "MCL-T4-Q30-v1", info, 8, kd, 96);
...
int main(int argc, char** argv) {
    ...
    uint8_t pre[80]; for (int i = 0; i < 4; i++) put32(pre + 4*i, s.t[i]); std::memcpy(pre + 16, hx, 32); put64(pre + 48, N); static const char otag[24] = "MCL-VDF128-T4-v2-out\0\0\0"; std::memcpy(pre + 56, otag, 24);
    uint8_t y[32]; sha256(pre, 80, y); std::printf("preimage     %s\ny            %s\n", hex(pre,80).c_str(), hex(y,32).c_str());
    const char* want_y = "1d0f60cc602b12edbd1177cedb5bb19864f3a890c50b6a2aab028017f3160848";
    bool ok = hex(y,32) == want_y && s.t[0] == 0xbafabcaau && s.t[1] == 0x1e0d2231u && s.t[2] == 0xbec6cfe4u && s.t[3] == 0xb5d82436u;
    std::printf("Vector 5 (v2): %s\n", ok ? "REPRODUCED (final state and y match the paper)" : "MISMATCH");
    return ok ? 0 : 1;
}
