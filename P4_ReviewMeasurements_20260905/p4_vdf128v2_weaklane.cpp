...
// P4 review measurement 2026-09-05, round 5 item R5-5: WEAK-LANE INSTANCES of VDF128-T4 v2.
...
int main() {
    std::printf("VDF128-T4 v2 weak-lane instance probe (MCL-VDF128-WEAKLANE-2026-0905-001; engine %d.%d.%d)\n", MCL_VERSION_MAJOR, MCL_VERSION_MINOR, MCL_VERSION_PATCH);
    ...
    for (uint64_t i = 0; i < CAP; i++) {
        ...
        MCL_Q30_Sextet W = mcl_vdf128v2_weights((const uint8_t*)buf, (size_t)L); uint32_t w[12]; lanes(W, w); tried++;
    ...
    probes("control: battery input", X, mcl_vdf128v2_weights((const uint8_t*)X, std::strlen(X)));
    if (!best_lt20.empty()) probes("lane < 2^20 (~1% of inputs)", best_lt20, mcl_vdf128v2_weights((const uint8_t*)best_lt20.data(), best_lt20.size()));
    if (!best_lt16.empty()) probes("lane < 2^16", best_lt16, mcl_vdf128v2_weights((const uint8_t*)best_lt16.data(), best_lt16.size()));
    if (!best_le15.empty()) probes("lane <= 15 (ground)", best_le15, mcl_vdf128v2_weights((const uint8_t*)best_le15.data(), best_le15.size()));
    if (!best_pair16.empty()) probes("both lanes of one pair < 2^16 (ground)", best_pair16, mcl_vdf128v2_weights((const uint8_t*)best_pair16.data(), best_pair16.size()));
