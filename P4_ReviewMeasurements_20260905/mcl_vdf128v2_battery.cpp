/*
 * ============================================================================
 * mcl_vdf128v2_battery.cpp — VDF property + falsification battery for VDF128_T4 v2 (per-input weights)
 * Doc ID: MCL-VDF128-BATTERY-2026-0905-002 (v2 of MCL-VDF128-BATTERY-2026-0817-001: weights = params_from_key(SHA-256(x)); PART 3 structural probes added)
 * ============================================================================
...
int main() {
    std::printf("================================================================\n");
    std::printf("  VDF128_T4 v2 (per-input weights) — property + falsification battery + structural probes\n");
    std::printf("  %s   engine: additive over mcl_core v8.1.0 + sidecar v1.0.4\n", "MCL-VDF128-BATTERY-2026-0905-002");
    std::printf("================================================================\n");
    const char* X = "MCL-VDF128-battery-input";
    const size_t XL = std::strlen(X);
    const MCL_Q30_Sextet W = mcl_vdf128v2_weights((const uint8_t*)X, XL);   // v2: the map instance F_x of THIS input
    const int64_t kp = mcl_q30_K_phase(K_DEFAULT);
    ...
    // P1 timing linearity
    {
        ...
        mcl_vdf128v2_eval((const uint8_t*)X, XL, 100000, y);
        for (int i = 0; i < 5; i++) {
            double best = 1e30;
            for (int rep = 0; rep < 2; rep++) {           // min-of-2: reject scheduler noise
                auto t0 = Clock::now();
                mcl_vdf128v2_eval((const uint8_t*)X, XL, (uint64_t)Ns[i], y);
    ...
    // P3 deterministic reproducibility
    {
        ...
        mcl_vdf128v2_eval((const uint8_t*)X, XL, 100000, y0);
        for (int t = 0; t < 50; t++) {
            mcl_vdf128v2_eval((const uint8_t*)X, XL, 100000, yi);
    ...
    // P4 output depends on N
    {
        ...
        for (int i = 0; i < 6; i++) mcl_vdf128v2_eval((const uint8_t*)X, XL, Ns[i], ys[i]);
    ...
    // P7 configurable delay range
    {
        ...
        for (int i = 0; i < 6; i++) { mcl_vdf128v2_eval((const uint8_t*)X, XL, Ns[i], y); if (!y[0] && !y[31]) allok = false; }
    ...
    // A4 (OP4) output binding under the hash finalization
    {
        ...
        mcl_vdf128v2_eval((const uint8_t*)X, XL, 50000, y1);
        mcl_vdf128v2_eval((const uint8_t*)X, XL, 50001, y2);          // N+1
        const char* X2 = "MCL-VDF128-battery-inpuT";                 // 1-char change
        mcl_vdf128v2_eval((const uint8_t*)X2, std::strlen(X2), 50000, y3);
