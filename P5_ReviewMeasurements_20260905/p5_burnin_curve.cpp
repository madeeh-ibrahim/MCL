 ...
 * Doc ID: MCL-P5-BURNIN-2026-0905-001
...
static void trial_bytes(const char*tag,uint64_t i,uint8_t out[32]){uint8_t buf[40];std::memset(buf,0,32);std::strncpy((char*)buf,tag,31);for(int k=0;k<8;k++)buf[32+k]=(uint8_t)(i>>(k*8));CC_SHA256(buf,sizeof(buf),out);}
...
int main(){
  ...
  // T2 avalanche
  double s2=0;int mn2=256,mx2=0; for(int i=0;i<5000;i++){Tx t=tx_of(2000000+i);Tag a=tag_v3(A,t,V1);Tx t2=t;int bit=i%64;t2.value^=(1ULL<<bit);Tag b=tag_v3(A,t2,V1);int h=ham(a,b);s2+=h;mn2=std::min(mn2,h);mx2=std::max(mx2,h);}
