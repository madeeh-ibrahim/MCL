...
**Addendum (2026-09-05 evening, referee check) — common burn-in before the split.** `decorr_time` gained `DECORR_BURNIN=<steps>` (B common iterations under parameter set A before the two trajectories diverge). (3,5), K = 12, GS, 16,384 seeds, B = 10⁴, both perturbation axes (`res_omega_35_K12_gs_burnin1e4_*`, `res_K_35_K12_gs_burnin1e4_*`): growth rates 5.74–5.79 (λ₁ = 5.778), and **t_sat and robust t_dec identical to the no-burn-in runs at every δ** (ω: t_dec 8/7/6/5/4/3/3; K: 7/7/6/5/4/3/2). Starting at the seed phases rather than on the attractor does not affect the law. **Note on δ₁ for a K change:** exp⟨ln d(1)⟩ − ln ΔK = +2.26 (≈ 9.6 ΔK) — the sequential substitution carries the θ₁ difference into θ₂ within the same iteration (factor of order pK); the v3 text of §IV.D was corrected accordingly (it had said "(2/π)ΔK").
...
**Correction (2026-09-05 late night, referee-precision pass).** (1) The intercept of Eq. (9) is **t₀ ≈ 2–3 iterations**, not 1–2 as written above and in the first v3 text: with the slope fixed at 1/λ₁ (robust 4σ t_dec, δ₁ from ⟨ln d(1)⟩), the configuration means are 2.2–3.2 (overall 2.7; individual values 1.75–3.74); the saturation time alone has intercept ≈ 1.7 (one iteration above the naive ln(1/δ₁)/λ₁ extrapolation — alignment of the injected perturbation with the leading direction), and the correlation collapses ≈ 1 iteration after saturation. (2) ΔK = 10⁻³ was **never run** (the K list is 10⁻¹², 10⁻¹⁰, 10⁻⁸, 10⁻⁶, 10⁻⁴, 10⁻², 10⁻¹); the measured values are t_dec = 4 for ΔK = 10⁻⁴ and 3 for 10⁻²; the paper text quoting 10⁻³ was corrected. (3) Sensitivity: t_dec with a 3σ or 5σ floor differs from the 4σ value by at most one iteration in one δ; cos θ₂ or sin θ₁ in place of cos θ₁ changes it by at most one iteration (`decorr_time` steps files, `r_cos2`, `r_sin1` columns).

## E. Provenance pass of 2026-09-06 — every number of §V.B–§V.G, §VI, §VII checked against a raw record

Automated extraction of all numeric tokens in the results sections and search in the public bundle + measurement folders + Paper 1: every value was found in a record **except** (i) the Bonferroni thresholds 2.63 × 10⁻⁷ / 8.80 × 10⁻⁷ (arithmetic, fine); (ii) the 10⁵–5 × 10⁶-channel rows of Table IV; (iii) all rows of Table VI except K = 0.5 and K = 12.

**Table IV (channel scaling).** The rows are verbatim in the Stage-1 record `MCL Project/Main Core/20260508_Results/MCL_Scale_v2.2.0_Test_Results.md` (runs A–D, `mcl_scale` v2.2.0, 13–19 May 2026; 100 ch exhaustive 4,950 pairs; 1,000–5 × 10⁶ ch sampled 5,000 pairs; max|r| 0.039154 / 0.122386 / 0.264694, sampled bounds 0.040212 / 0.128561 / 0.302745, min entropy 7.972998 / 7.698713 / 6.528820; run C total 5,674.3 s, 4,294.4 s at the 5 × 10⁶ point; 24 of 24 quantities identical across two Macs). The record was **not** in the public bundle → copied to `results/MCL_Scale_v2.2.0_Test_Results_20260519.md` (v0.2.8 delta) and cited in the paper's Table IV note; the paper's runtime phrasing was made exact.

**Table VI (λ vs K for (2,3), 3 seeds).** No raw record anywhere (the values appear only in old paper drafts). **Re-measured 2026-09-06** (`tableVI_lyap/`, record **MCL-P3-TABLEVI-2026-0906-001**): engine `compute_lyapunov`, seeds 12345678901234 / 98765432109876 / 31415926535897, 10⁷ QR iterations each. Means (λ₁, λ₂): K = 0.5: −0.4481, −0.5327 (identical to the old table); 1.0: 0.8162, 0.2406; 2.0: 1.6247, 0.5753; 5.0: 3.2652, 0.7830; 8.0: 4.1819, 0.7997; 12: 4.9706, 0.8059; 20: 5.9965, 0.8091; 50: 7.8237, 0.8107. Differences from the old table reach 0.002 (K = 2, 20, 50) — larger than the ±0.0004 per-seed SE, i.e. the old table used other seeds or fewer iterations; its message (logarithmic λ₁ growth, λ₂ plateau ≈ 0.81 = 2 ln 1.5) is unchanged. **The paper's Table VI now carries the new values with seeds and record ID.**
